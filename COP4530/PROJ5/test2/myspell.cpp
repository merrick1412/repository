...
unrankedWords.resize(misspelledWords.size());
    ...
    for (int i = 0; i < misspelledWords[i].size(); i++){
        string capitalizedString;
        for (char c : misspelledWords[i][0]) {
            capitalizedString += std::toupper(c);
        }
        misspelledWords[i][0] = capitalizedString;
    }
    for (int i = 0; i < suggestionsList.size(); i++){
        int choice;
        cout<<endl<<"==========================="<<endl;
        cout<<"Misspelled word: "<<misspelledWords[i][0]<<endl;
